...
module.exports = {
    ...
    async execute(react, user) {
        if (user.bot) return;
        const getdata = await sql(`select * from job_message where messageid="${react.message.id}";`);
        if (!getdata[0]?.messageid) return;
        const react_channel = await react.message.guild.channels.fetch(getdata[0]?.channelid);
        const react_message = await react_channel.messages.fetch(getdata[0]?.messageid);
