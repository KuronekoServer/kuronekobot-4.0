...
(async () => {
    let speechlist = [];
    let list;
    const voicevox = await getJson(`http://127.0.0.1:${process.env.voicevox}/speakers`);
    const COEIROINK = await getJson(`http://127.0.0.1:${process.env.COEIROINK}/speakers`);
    const SHAREVOX = await getJson(`http://127.0.0.1:${process.env.SHAREVOX}/speakers`);
