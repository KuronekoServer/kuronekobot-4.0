...
module.exports = {
    ...
    async execute(command) {
        ...
        const promises = dataNames.map((name) => {
            return sql.select(name, `guildid = ${command.guild.id}`)
                .then((res) => {
                    if (!res) data[name] = {};
        ...
        const normalFieldData = [
            ["Ticketlogチャンネル", `${data.ticket_channel?.guildid ? command.guild.channels.cache.get(data.ticket_channel?.channelid) || data.ticket_channel?.channelid : "未設定"}`],
            ["logチャンネル", `${data.log_channel?.guildid ? command.guild.channels.cache.get(data.log_channel?.channelid) || data.log_channel?.channelid : "未設定"}`],
            ["役職パネル", `${data.job_message?.guildid ? `対象チャンネル:${command.guild.channels.cache.get(data.job_message?.channelid) || data.log_channel?.channelid}\n対象メッセージ:[messagelink](https://discord.com/channels/${data.log_channel?.guildid}/${data.log_channel?.channelid}/${data.log_channel?.messageid})` : "未設定"}`],
        ];
