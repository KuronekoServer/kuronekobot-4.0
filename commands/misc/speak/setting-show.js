    ...
    if (ope === "server") {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '自動入室チャンネル', value: data[0]?.auto_voice_channel ? `${interaction.guild.channels.cache.get(data[0]?.auto_voice_channel) || "取得失敗"}=>${interaction.guild.channels.cache.get(data[0]?.auto_text_channel) || "取得失敗"}` : "なし", inline: true },
            )
