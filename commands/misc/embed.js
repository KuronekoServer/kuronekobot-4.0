...
module.exports = {
    ...
    async execute(interaction) {
        ...
        interaction.reply({ content: `${channel} で埋め込みメッセージを送ります`,  ephemeral: true });
    ...
    async function embedSetup(channel, member) {
        ...
        if (!btnInteraction) return sentMsg.edit({ content: "応答がなかったため埋め込み作成を停止したよ～", components: [] });
        ...
        if (!modal) return sentMsg.edit({ content: "応答がなかったため埋め込み作成を停止したよ～", components: [] });
      
        modal.reply({ content: "埋め込みを送信したよ～", ephemeral: true }).catch((ex) => {});
        ...
        if (!title && !author && !description && !footer)
          return sentMsg.edit({ content: "おい！空の埋め込みは送信できないぞ！ :rage:", components: [] });
        ...
        collector.on("collect", async (interaction) => {
          if (interaction.customId === "EMBED_FIELD_ADD") {
            ...
            modal.reply({ content: "フィールドを追加したぞ！", ephemeral: true }).catch((ex) => {});
          ...
          // remove field
            ...
            if (fields) {
              fields.pop();
              embed.setFields(fields);
              interaction.reply({ content: "フィールドを削除したぞ！", ephemeral: true });
            } else {
              interaction.reply({ content: "削除できるフィールドがないぞ！", ephemeral: true });
            }
          }
